  ...
  for (const step of job.steps) {
    ...
    for (const dd of step.dds) {
      if (dd.name) {
        ...
        if (seen.has(key)) {
          diagnostics.push(makeDiagnostic('JCLD001', { name: dd.name, step: step.name }, { line: dd.line, column: 1 }));
        } else {
          seen.set(key, dd.line);
        }
      }
      checkDataSource(dd, step, diagnostics);
    }
  }
}
...
function checkDataSource(dd, step, diagnostics) {
  ...
  const sources = [];
  if (hasDSN) sources.push('DSN');
  if (hasDummy) sources.push('DUMMY');
  if (hasSysout) sources.push('SYSOUT');
  if (hasStar || hasData) sources.push('instream data');
  if (hasLike) sources.push('LIKE');

  if (sources.length === 0 && !dd.instreamData) {
    diagnostics.push(makeDiagnostic('JCLD002', {}, { line: dd.line, column: 1 }));
  } else if (sources.length > 1) {
    diagnostics.push(makeDiagnostic('JCLD003', { a: sources[0], b: sources[1] }, { line: dd.line, column: 1 }));
  }
}
