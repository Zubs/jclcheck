...
function unbalancedParenCheck(text) {
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    if (ch === '(') depth++;
    else if (ch === ')') { depth--; if (depth < 0) return { ok: false, at: i }; }
  }
  if (depth !== 0 || inQuote) return { ok: false, at: text.length };
  return { ok: true };
}
...
function tokenizeParams(operand, baseCol = 1) {
  ...
  for (const part of parts) {
    ...
    if (eqIdx === -1) {
      params.push({ kind: 'positional', value: trimmed, col });
    } else {
      ...
      params.push({ kind: 'keyword', key: key.toUpperCase(), rawKey: key, value: valueRaw, subvalues, col, valueCol });
    }
  }
...
module.exports = { tokenizeParams, splitTopLevel, parseSubvalues, unbalancedParenCheck };
