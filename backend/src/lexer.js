...
function splitFields(line) {
  ...
  return { name, op, rest, nameCol, opCol, restCol };
}
...
function needsContinuation(operand) {
  ...
  return { continues: trailingComma || depth > 0 || inQuote, openParens: depth, openQuote: inQuote, trailingComma };
}
...
function lex(source) {
  ...
  while (i < lines.length) {
    ...
    if (!isJclLine(rawLine)) {
      diagnostics.push(makeDiagnostic('JCLL001', {}, { line: lineNo, column: 1, source: 'lexer' }));
      i++;
      continue;
    }
    ...
    if (isCommentLine(rawLine)) {
      statements.push({
        type: 'COMMENT', text: rawLine.slice(2), line: lineNo, endLine: lineNo, raw: [rawLine]
      });
      i++;
      continue;
    }
    ...
    if (rawLine.trimEnd() === '//') {
      ...
      statements.push({ type: 'NULL', line: lineNo, endLine: lineNo, raw: [rawLine] });
      i++;
      continue;
    }

    if (rawLine.length > 80) {
      diagnostics.push(makeDiagnostic('JCLL002', {}, { line: lineNo, column: 81, source: 'lexer' }));
    }

    const { name, op, rest, nameCol, opCol, restCol } = splitFields(rawLine);
    ...
    while (cont.continues) {
      ...
      if (nextIdx0 >= lines.length || !isJclLine(lines[nextIdx0]) || isCommentLine(lines[nextIdx0]) || looksLikeNewStatement) {
        if (cont.openQuote) {
          diagnostics.push(makeDiagnostic('JCLL003', {}, { line: lastConsumedLineNo, column: 1, source: 'lexer' }));
        } else {
          diagnostics.push(makeDiagnostic('JCLP004', {}, { line: lastConsumedLineNo, column: 1, source: 'parser' }));
        }
        break;
      }
    ...
    statements.push({
      type: 'STMT',
      name, op, operand, comment,
      nameCol, opCol, operandCol,
      line: lineNo,
...
module.exports = { lex, splitFields, splitOperandAndComment, needsContinuation, isCommentLine, isJclLine };
