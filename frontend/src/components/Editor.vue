...
const lineSeverityMap = computed(() => {
  ...
  for (const d of props.diagnostics) {
    ...
    if (!cur || rank[d.severity] > rank[cur]) map.set(d.line, d.severity);
  }
  return map;
});
...
function scrollToLine(line) {
  ...
  el.scrollTop = target;
  if (gutterEl.value) gutterEl.value.scrollTop = target;
}
...
.editor-topline__label { color: var(--field-turquoise); font-weight: 600; }
...
.editor-gutter__row.is-error { color: var(--error); }
.editor-gutter__row.is-warning { color: var(--warning); }
.editor-gutter__row.is-info { color: var(--info); }
.editor-gutter__dot {
